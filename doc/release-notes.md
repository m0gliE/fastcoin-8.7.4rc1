0.8.7.4 changes
=============
- Checkpoint update to block 3621535. 
    - Included three new checkpoints.
- Image enhancements 
     -Modified the nsis-header image size to correct dimensions. Colored send arrows.)
- Mac and Windows Official Gitian Builds: upgrade to openssl-1.0.1i
...
- Minor fixes:
  - s/Bitcoin/Fastcoin/ in the Coin Control example
...
- Internal miner added back to Fastcoin.  setgenerate now works, although it is generally a bad idea as it is significantly slower than external CPU miners.
...
Fix out-of-bounds check (Fastcoin currently does not use this codepath, but we apply this
patch just to match Bitcoin 0.8.5.)

0.8.4.1 changes
===============

CVE-2013-5700 Bloom: filter crash issue - Fastcoin 0.8.3.7 disabled bloom by default so was 
...
nodes running with the -debug flag to crash, although it was lot less likely on Fastcoin 
